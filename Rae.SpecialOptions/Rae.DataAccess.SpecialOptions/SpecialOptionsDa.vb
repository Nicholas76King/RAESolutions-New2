   ...
   Public Class SpecialOptionsDa
      ...
      ByVal assignedBy As String, ByVal assignedTo As String, ByVal expirationDate As Date) As Integer
         ...
         Try
            ...
            sql = "SELECT MAX([" & T.Id.ToString & "]) FROM [" & T.TableName & "]"
            command = New OleDbCommand(sql, connection, transaction)
         ...
         Try
            ...
            Else
               While reader.Read
                  ...
                  price = CNull.ToDouble(reader(T.Price))
                  assignedBy = reader(T.AssignedBy).ToString
                  assignedTo = reader(T.AssignedTo).ToString
                  expirationDate = CNull.ToDate(reader(T.ExpirationDate), defaultExpirationDate)
                  dateGenerated = CNull.ToDate(reader(T.DateGenerated))
                  found = True
               End While
            End If
         Catch ex As OleDbException
      ...
      ''' </summary>
      Public Shared Sub Update(ByVal description As String, ByVal price As Double, _
      ...
      Public Shared Function Verify(ByVal id As Integer, ByVal price As Double) As String
         ...
         sql.AppendFormat("SELECT [{3}] FROM [{0}] WHERE [{1}] = {2}", _
            T.TableName, T.Id, id.ToString, T.Price)
      ...
      ' TODO: RetrieveByGeneratedBetween
      Public Shared Function RetrieveByGeneratedBetween(ByVal beginningDate As Date, ByVal endingDate As Date) As DataTable
