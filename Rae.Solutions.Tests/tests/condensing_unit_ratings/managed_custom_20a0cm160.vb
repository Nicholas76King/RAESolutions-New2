...
Public Class managed_custom_20a0cm160
   ...
   Sub init
      ...
      Dim cu = repository.get_unit(model:="20A0CM160")
        ...
        conditions.fan_file_name_1 = Rae.RaeSolutions.Business.Intelligence.FanIntel.SelectStandardFile(cu.circuits(0).fan_diameter, conditions.altitude, cu.circuits(0).hp)
        conditions.fan_file_name_2 = Rae.RaeSolutions.Business.Intelligence.FanIntel.SelectStandardFile(cu.circuits(1).fan_diameter, conditions.altitude, cu.circuits(1).hp)
   ...
   Sub condenser_temp_should_be_126p7
      ...
      Assert.IsTrue( is_accurate(condenserTemp, tolerance, 126.9) )
   End Sub
   
   <TestMethod> _
   Sub td_should_be_31p7
      Dim td = result.point.td
      Assert.IsTrue( is_accurate(td, tolerance, 31.9) )
   End Sub
   
   <TestMethod> _
   Sub capacity_should_be_207p66
      Dim capacity = result.point.capacity
      Assert.IsTrue( is_accurate(actual:=capacity, percentage:=tolerance, expected:=207.43) )      
   End Sub
   
   <TestMethod> _
   Sub power_should_be_210p51_kw
      Dim kw = result.point.unit_kw
      Assert.IsTrue( is_accurate(actual:=kw, percentage:=tolerance, expected:=210.81) )
   End Sub
   
   <TestMethod> _
   Sub eer_should_be_11p84
      Dim eer = result.point.unit_eer
      Assert.IsTrue( is_accurate(actual:=eer, percentage:=tolerance, expected:=11.81) )
   End Sub
   
   <TestMethod> _
   Sub amps_at_230_should_be_650
      Dim ampsAt230 = result.point.unit_amps
      Assert.IsTrue( is_accurate(actual:=ampsAt230, percentage:=tolerance, expected:=651) )
   End Sub
   
   <TestMethod> _
   Sub condenser_capacity_should_be_235p32
      Dim capacity = result.point.condenser_capacity
      Assert.IsTrue( is_accurate(actual:=capacity, percentage:=tolerance, expected:=235.12) )
   End Sub
