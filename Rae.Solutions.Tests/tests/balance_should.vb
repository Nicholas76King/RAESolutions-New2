...
Public Class balance_should
   ...
   Sub match_original_DS10H2
      ...
      conditions.voltage = 230
        conditions.fan_file_name_1 = FanIntel.SelectStandardFile(cu.circuits(0).fan_diameter, conditions.altitude, cu.circuits(0).hp)
      ...
      Dim expected = 10.95
      IsTrue(is_accurate(p.unit_eer, 0.1, expected), "eer is off by: %" & (p.unit_eer - expected) / expected * 100)

      expected = 156557.2
      IsTrue(is_accurate(p.condenser_capacity, 0.1, expected), "condenser capacity off by: %" & (p.condenser_capacity - expected) / expected * 100)

      expected = 134334.7
      IsTrue(is_accurate(p.capacity, 0.1, expected), "capacity off by: %" & (p.capacity - expected) / expected * 100)
   End Sub
   ...
   Sub match_original_20A0CD10_balance
      ...
      conditions.hp_2         = cu.circuits(1).hp
        conditions.fan_file_name_1 = FanIntel.SelectStandardFile(cu.circuits(0).fan_diameter, conditions.altitude, cu.circuits(0).hp)
        conditions.fan_file_name_2 = FanIntel.SelectStandardFile(cu.circuits(1).fan_diameter, conditions.altitude, cu.circuits(1).hp)
      ...
      Dim expected = 12.5
      IsTrue( is_accurate(pt.condenser_capacity, 0.1, expected) )
      
      expected = 10.52
      IsTrue( is_accurate(pt.unit_eer, 0.1, expected) )
   End Sub
