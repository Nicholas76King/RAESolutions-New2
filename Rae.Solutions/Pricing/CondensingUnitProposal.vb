...
Class CondensingUnitProposal
    Private screen As condensing_unit_pricing_screen
    Private report_file_path As String

    Sub New(ByVal screen As condensing_unit_pricing_screen)
        Me.screen = screen
        Me.report_file_path = CondensingUnitProposalFilePath
    ...
    Private Sub show_word_report()
        ...
        Dim spec_bag = New CondensingUnit_proposal_grabber(screen.specsControl).grab()
        ...
        Dim report = New Rae.reporting.beta.report(reports.file_paths.CondensingUnitProposalFilePath)
    ...
    Private Function BuildFilteredOptions(ByVal options As SelectedOptionsDataTable) As List(Of DataTable)
        ...
        For Each r1 In options
            ...
            Dim categoriesContains As Boolean = False
            For Each cat1 As String In categories
                If cat1.ToLower = r1.Category.ToLower Then
                    categoriesContains = True
                    Exit For
                End If
            Next

            If categoriesContains = False Then categories.Add(r1.Category)
    ...
    '    If pricing_bag.warranty > 0 Then
    '        work_count += 1
    '        startupAndCompNOTE &= work_count & ".   Extended compressor warranty for a total of (5) years" & new_line & vbTab & "     (parts only -- labor, crane and refrigerant costs are not included.)"
    '    End If
    ...
    Private Sub setPricingParametersIn(ByVal pricing_bag As pricing_grabber.bag, ByVal equipment_bag As equipment_grabber.bag, ByRef text As Dictionary(Of String, String))
        ...
        Dim work_count = 3
        Dim startupAndCompNOTE = ""
        If pricing_bag.start_up <> "$0" AndAlso Not equipment_bag.model.StartsWith("20") Then
            work_count += 1
            startupAndCompNOTE = work_count & ".  Supervision of Startup procedures by factory personnel" & new_line
        Else
        ...
        If Not String.IsNullOrEmpty(pricing_bag.start_up) AndAlso IsNumeric(pricing_bag.start_up) Then
            If pricing_bag.start_up = 0 Then
                text.Add("startup", "Not Inc.")
            Else
                text.Add("startup", pricing_bag.start_up)
            End If
        Else
            text.Add("startup", "Contact Factory")
        End If
        ...
        End If
        'text.Add("startup_note", start_up_note)
        ...
        ' Dim compressor_note = ""
        If Not String.IsNullOrEmpty(pricing_bag.warranty) AndAlso IsNumeric(pricing_bag.warranty) AndAlso pricing_bag.warranty > 0 Then
            work_count += 1
    ...
    'Private Sub set_spec_parameters_in(ByVal report As i_report, ByVal bag As COndensingUnit_proposal_grabber.bag)
    '    Dim specs_control = CType(screen.specsControl, CondensingUnitSpecsControl)
    ...
    '    report.pass("Capacity1", bag.CapacityCircuit1)
    '    report.pass("Capacity2", bag.CapacityCircuit2)
    '    report.pass("Capacity3", bag.CapacityCircuit3)
    '    report.pass("Capacity4", bag.CapacityCircuit4)
    '    report.pass("SuctionTemp", bag.SuctionTemp)
    '    report.pass("EvapTemp", bag.EvapTemp)
    '    report.pass("Altitude", bag.altitude)
    ...
    Private Sub setSpecParametersIn(ByVal bag As CondensingUnit_proposal_grabber.bag, ByRef text As Dictionary(Of String, String))
        Dim specs_control = CType(screen.specsControl, CondensingUnitSpecsControl)
        Dim tag_length = If(bag.tag.Length > 25, 25, bag.tag.Length)
        ...
        text.Add("Capacity", bag.CapacityCircuit1)

        text.Add("Capacity1", bag.CapacityCircuit1)
        text.Add("Capacity2", bag.CapacityCircuit2)
        text.Add("Capacity3", bag.CapacityCircuit3)
        text.Add("Capacity4", bag.CapacityCircuit4)
        text.Add("UnitEfficiency", bag.efficiency)

        text.Add("SuctionTemperature", bag.SuctionTemp)
        text.Add("EvapTemp", bag.EvapTemp)
